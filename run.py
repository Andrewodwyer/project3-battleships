        ...
        for s in range(100):  # Try up to 100 times to place the ship, without overlapping or going off grid
            ...
            if direction == 'horizontal':
                ...
                if all(grid[row][col+j] == ' ' for j in range(size)): #checks if grid row and column coordinates are ' ' empty
                    for j in range(size):
                        grid[row][col+j] = Fore.WHITE + Style.BRIGHT + Back.WHITE +ship_type[0].upper() + Style.RESET_ALL #color ship and first letter from name of ship
                ...
                if all(grid[row+i][col] == ' ' for i in range(size)):
                    for i in range(size):
                        grid[row+i][col] = Fore.WHITE + Style.BRIGHT + Back.WHITE +ship_type[0].upper() + Style.RESET_ALL  #ship_type[0].upper() takes the first letter of the ship type and makes the letter uppercase
                    break
